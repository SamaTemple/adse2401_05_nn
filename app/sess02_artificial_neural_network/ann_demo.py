...
===========================================================
This program implements a simple feed-forward neural network
with one hidden layer, trained for manual forward propagation,
backwards propagation and gradient descent, to learn the
 XDR function,
...
#=====================================================
#1. Sigmoid functions and sigmoid derivative
#=====================================================
...
class NeuralNetwork:
    ...
    def __init__(
        ...
        self.weights_input_hidden: np.ndarray = rng.uniform(
            -1.0, 1.0, (self._input_size, self._hidden_size)
        ...
        self.weights_hidden_output: np.ndarray = rng.uniform(
            -1.0, 1.0, (self._input_size, self._output_size)
        )
        self.bias_output: np.ndarray = np.zeros((1, self._output_size))

        # Placeholders that will store intermedate values computed
        # during forward propagation. These required again during
    ...
    def forward_propagation(self, x: np.ndarray) -> np.ndarray:
        ...
        self.hidden_weighted_sum = (input @ self.weights_input_hidden + self.bias_hidden )
    ...
    @staticmethod
    def calculate_loss(prediction: np.ndarray, target: np.ndarray) -> float:
        return float(np.mean (targets - prediction) ** 2)
